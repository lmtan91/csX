...
void drawTreeOrder(GWindow& gw, GPoint point, double size, int currOrder, int order, int angle)
{
    if(currOrder >= order)
    {
        return;
    }
    gw.setColor(currOrder < order - 1 ? BRANCH_COLOR : LEAF_COLOR);
    GPoint end = gw.drawPolarLine(point, size, angle);

    drawTreeOrder(gw, end, size / 2, currOrder + 1, order, angle - 45);
    drawTreeOrder(gw, end, size / 2, currOrder + 1, order, angle + 45);
    drawTreeOrder(gw, end, size / 2, currOrder + 1, order, angle - 30);
    drawTreeOrder(gw, end, size / 2, currOrder + 1, order, angle + 30);
    drawTreeOrder(gw, end, size / 2, currOrder + 1, order, angle - 15);
    drawTreeOrder(gw, end, size / 2, currOrder + 1, order, angle + 15);
    drawTreeOrder(gw, end, size / 2, currOrder + 1, order, angle - 0);
}
...
void drawTree(GWindow& gw, double x, double y, double size, int order) {
    // TODO: write this function
    drawTreeOrder(gw, GPoint(x + size / 2, y + size), size / 2, 0, order, 90);
}
